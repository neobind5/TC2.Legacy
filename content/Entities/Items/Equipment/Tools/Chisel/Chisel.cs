...
{
	public static class ChiselOverride
	{
		...
		public struct Data: IComponent
		{
			...
			public ChiselOverride.Data.Flags flags;

			[Save.Ignore, Net.Ignore] public float t_next_use;
		}

		public struct UseRPC: Net.IRPC<ChiselOverride.Data>
		{
			public Vector2 world_position;
			public Vector2 direction;

#if SERVER
			public void Invoke(Net.IRPC.Context rpc, ref ChiselOverride.Data data)
			{
		...
		public struct ChiselGUI: IGUICommand
		{
			...
			public ChiselOverride.Data chisel;
				...
				using (var hud = GUI.Window.HUD("chisel.hud"u8))
				{
					if (hud.show)
					{
						...
						if (!GUI.IsHovered)
						{
							...
							{
								var rpc = new ChiselOverride.UseRPC()
								{
							...
							//	if (nearest_index >= 0)
							//	{
							//		var rpc = new ChiselOverride.MarkRPC()
							//		{
		...
		[Source.Parent] in Interactor.Data interactor, [Source.Owned] ref ChiselOverride.Data chisel,
		...
		[Source.Owned] ref ChiselOverride.Data chisel, [Source.Owned] in Transform.Data transform, 
