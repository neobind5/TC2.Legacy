...
namespace TC2.Base.Components
{
	public static class Radioactive
	{
		...
		public struct Data: IComponent
		{
			...
			public float damage_max = 25.00f;
			public float size = 4.00f;
			public float delay = 0.50f;
			public float delay_variation = 0.00f;
			public float total_damage = 1.00f;
			public float next_update;
			public Data()
			{
			}
		}
		...
		public static void Update(ISystem.Info info, Entity entity, ref XorRandom random, ref Region.Data region, [Source.Owned] in Transform.Data transform, [Source.Owned] ref Radioactive.Data radioactive, [Source.Owned] ref Resource.Data resource)
		{
			if (resource.IsNotNull())
			{
				var material = resource.material.GetData();
				if (material.IsNotNull())
				{
					radioactive.total_damage = radioactive.damage + (MathF.Log2(1.00f + (resource.quantity / Maths.Max(material.quantity_max, 1.00f))) * (radioactive.damage_max-radioactive.damage));
				}
			}
			else
			{
				radioactive.total_damage = radioactive.damage;
			}
			if (info.WorldTime >= radioactive.next_update)
			{
				var ts = Timestamp.Now();
				Span<OverlapResult> results = stackalloc OverlapResult[8];
				if (region.TryOverlapPointAll(transform.position, radioactive.size, ref results, mask: Physics.Layer.Destructible))
				{
					foreach (ref var result in results)
					{
						...
						(
							...
							velocity: result.gradient,
							normal: -result.gradient,
							damage_integrity: radioactive.total_damage,
							damage_durability: radioactive.total_damage,
							damage_terrain: radioactive.total_damage,
							target_material_type: result.material_type,
							damage_type: Damage.Type.Radiation,
							yield: 0.00f,
							size: 0.500f,
							impulse: radioactive.delay
						);
					}
				}
				//App.WriteLine($"{ts.GetMilliseconds():0.0000} ms");
				radioactive.next_update = info.WorldTime + radioactive.delay + (random.NextFloatRange(0f, 2f)-1)*radioactive.delay_variation;
			}
		}
#endif
	}
